...
The project is intentionally standalone. It is not tied to any one firmware,
sender, controller family, emulator, or game.
...
- an entitlement-sensitive macOS virtual-device publisher for AMFI-disabled
  testing
...
For AMFI-disabled testing, ad-hoc sign the bridge executable with the included
entitlement after each build:
...
For a network sender that waits for a Mac-side VHB2 HELLO, bind the helper on
the LAN-facing interface and point it at the sender:
...
`--udp-source` is the remote sender address; the helper sends a VHB2 HELLO there
once per second and receives HID device/report packets back on the same UDP
socket.
...
--udp-source HOST   Send VHB2 HELLO to a UDP source, HOST[:PORT]
```
...
The first UI is intentionally plain. It provides:
...
- USB CDC and USB vendor-class devices are byte transports, not game
  ...
  framing protocol.
- UDP has no descriptor or controller schema. The bridge sends a descriptor once
  at device creation, then raw HID input/output/feature reports with device IDs,
  report IDs, sequencing and timestamps.

The wire protocol also retains an optional semantic message for software that
...
- `src/protocol.cpp`: VHB2 parser/builder
...
- `include/vhid/wire.h`: C-compatible sender-side wire constants
